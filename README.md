...
> independently. For Peach API questions that aren't about this plugin's code, see
> [developer.peachpayments.com](https://developer.peachpayments.com).

## Why reach for this instead of a quick custom provider

A payment provider is easy to write and easy to get subtly, expensively wrong. This one carries the
guards you'd want from any integration, made explicit:

- **Amount-integrity gate.** `authorizePayment` cross-checks the amount Peach reports on `/status`
  against the amount the session was created for, and fails closed on any mismatch or a missing amount.
  A result code alone never completes an order.
- **Webhook re-confirmation.** The webhook handler never acts on the notification body's claimed outcome
  or amount. It re-fetches `/status` by `checkoutId` and treats that as the only source of truth — so the
  result is independent of anything in the (forgeable) notification.
- **Fail-closed result mapping.** `000.400.101` / `000.400.102` (3DS "not participating" / "not enrolled")
  map to `error`, not success — they're intermediate, not terminal. Real Checkout V2 successes land on a
  terminal `000.100.1*` code.
- **Declined refunds aren't silent.** Peach returns HTTP 200 for a *declined* refund; the provider checks
...
The runtime provider id is `pp_<identifier>_<id>` — with `id: "peach"` that's **`pp_peach_peach`**, which
...
<details>
<summary><b>All provider options (annotated) + environment variables</b></summary>
...
options: {
  ...
  // passes to Checkout.initiate(), so it reaches the browser — semi-public, but don't commit it.
  ...
  paymentType: process.env.PEACH_PAYMENT_TYPE,     // "DB" capture-now (default) | "PA" pre-auth
  merchantName: process.env.PEACH_MERCHANT_NAME,   // shown in the Peach checkout UI

  // Fallback currency, used only when a session/refund carries none. No ZAR default: with neither a
  // session currency nor a default set, the provider throws rather than guessing.
  ...
  // Overriding a decline/error code logs a warning once, since it loosens a fail-closed default.
}
```
...
registered provider. With `id: "peach"`, register this URL in the Peach Dashboard as your checkout's
webhook endpoint (and set it as `notificationUrl`):

```
https://your-backend.example.com/hooks/payment/pp_peach_peach
```

Peach surfaces a signing secret when you add the webhook — that becomes your `secretToken`. You don't need
a body parser: the provider verifies the signature from the raw body when it's preserved and reconstructs
the signed message from Medusa's already-parsed body when it isn't. Verification **fails closed** — an
...
form-encoded key-value pairs** (not nested JSON) with a V1 HMAC over the sorted keys — a genuinely
...
after authorization and throws a clear error if it's missing rather than guessing.
...
Full list (challenge + decline scenarios): Peach's
[test and go-live reference](https://developer.peachpayments.com/docs/reference-test-and-go-live).

## Storefront integration

Backend-only — there's no npm storefront SDK because Peach's Checkout widget itself is a script-tag
global, not a package. [`examples/storefront/`](examples/storefront/) has reference React/Next.js code:
reading the session off the cart, mounting the embedded widget, and handling the return redirect.

## Limitations

- **Two-decimal currencies only.** Amounts are formatted/compared at 2 decimals; zero-decimal (JPY) and
  three-decimal currencies aren't supported, and magnitudes beyond ~9e13 minor units lose float precision.
- **No built-in ZAR default** — set `defaultCurrency` yourself if you want a fallback.
- **No server-side cancel.** Peach's V2 API can't cancel a checkout; `cancelPayment` is a no-op and unpaid
  checkouts expire after 30 minutes.
- **Capture is a no-op for `DB`.** Immediate-capture settles at Peach when the shopper pays. Pre-auth
  (`PA`) capture/void uses a separate Peach API this provider doesn't implement.
